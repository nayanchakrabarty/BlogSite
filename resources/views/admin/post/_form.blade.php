...
<div class="form-group">
    ...
    <div class="col-md-12">
        @php
            if (old("is_fetured")){
                $is_fetured = old('is_fetured');
            }elseif (isset($post)){
                $is_fetured = $post->is_fetured;
            }else{
                $is_fetured = null;
            }
        @endphp
        <input name="is_fetured" @if($is_fetured ==0) checked @endif value="0" type="radio" id="no"> <label for="no"> No</label>
        <input name="is_fetured" @if($is_fetured ==1) checked @endif value="1" type="radio" id="yes"> <label for="yes"> Yes</label>
        </div>
    @error('is_fetured')
    <div class="pl-1 text-danger">{{ $message }}</div>
    @enderror
...
<div class="form-group">
    <label class="col-md-12">Published at</label>
    <div class="col-md-12">
        <input name="published_at" value="{{ old('published_at',isset($post)?$post->published_at:null) }}" type="datetime-local" placeholder="Enter post published date" class="form-control form-control-line @error('published_at') is-invalid @enderror">
    </div>
    @error('published_at')
    <div class="pl-1 text-danger">{{ $message }}</div>
    @enderror
...
<div class="form-group">
    ...
    <div class="col-md-12">
        ...
        @endphp
        <input name="status" @if($status == 'published') checked @endif type="radio" value="published" id="published"><label for="published">Published</label>
        <input name="status" @if($status == 'unpublished') checked @endif type="radio" value="unpublished" id="unpublished"><label for="unpublished">Unpublished</label>
        <input name="status" @if($status == 'draft') checked @endif type="radio" value="draft" id="draft"><label for="draft">Draft</label>
    </div>
    @error('status')
    <div class="pl-1 text-danger">{{ $message }}</div>
    @enderror
